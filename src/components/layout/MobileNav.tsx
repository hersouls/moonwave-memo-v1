...
import { Hash, Star, StickyNote, Trash2, X } from 'lucide-react'
  ...
  return (
    <Transition show={isMobileMenuOpen} as={Fragment}>
      <Dialog onClose={closeMobileMenu} className="relative z-50 md:hidden">
        ...
        >
          <DialogPanel className="fixed inset-y-0 left-0 flex w-72 flex-col bg-white dark:bg-gray-900">
            {/* Header with profile */}
            ...
            <nav className="flex-1 overflow-y-auto px-2 py-3 scrollbar-none">
              ...
              >
                <StickyNote className="h-4.5 w-4.5 shrink-0" />
                <span className="flex-1 text-left">모든 메모</span>
                <span className="text-xs text-gray-400 dark:text-gray-500">{activeMemos.length}</span>
              </button>

              {/* Starred */}
              <button
                onClick={() => handleNav(() => setActiveFilter('starred'))}
                className={clsx(
                  'mb-0.5 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-colors',
              ...
              >
                ...
                <span className="flex-1 text-left">중요 메모</span>
              ...
              {trashFolder && (
                ...
                >
                  ...
                  <span className="flex-1 text-left">{trashFolder.name}</span>
